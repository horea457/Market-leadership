...
SKIP_1M = 21
START_12M = 252
END_7M = 147  # approx 7 trading months
...
def between_return(s, old_days, recent_days):
    """
    Return from old_days ago to recent_days ago.
    Example 252 -> 21 is standard-ish 12-1 formation window.
    """
    x = s.dropna()
    if len(x) <= old_days:
        return np.nan
...
def snapshot_group(group_name, dt):
    ...
    for _, meta in g.iterrows():
        ...
        for label, n in TRADING.items():
            tr = trailing_return(c[t], n)
            br = trailing_return(c[bench], n)
        ...
        tr = between_return(c[t], START_12M, SKIP_1M)
        br = between_return(c[bench], START_12M, SKIP_1M)
        row["return_12_1"] = tr
        row["benchmark_return_12_1"] = br
        row["excess_12_1"] = tr - br if pd.notna(tr) and pd.notna(br) else np.nan

        tr = between_return(c[t], START_12M, END_7M)
        br = between_return(c[bench], START_12M, END_7M)
        row["return_12_7"] = tr
        row["benchmark_return_12_7"] = br
        row["excess_12_7"] = tr - br if pd.notna(tr) and pd.notna(br) else np.nan

        consistency, monthly_mag = monthly_consistency(c[t], c[bench], 12)
        row["consistency_12m"] = consistency
        row["monthly_rel_magnitude_12m"] = monthly_mag

        # Prior drawdown excludes the latest 63 trading days to avoid
        # confusing the rebound itself with the preceding loss.
    ...
    # Cross-sectional ranks are central to the engine and reduce reliance
    # on arbitrary absolute return thresholds.
    rank_metrics = [
    ...
    # Deepest prior drawdown gets rank 1; strongest rebound gets rank 1.
    ...
    def old_leader_failure(rr):
        ...
        return bool(long_leader and current_weak and market_recovered)

    out["old_leader_failure_flag"] = out.apply(old_leader_failure, axis=1)

    # Rank migration.
    if "rank_126" in out.columns and "rank_63" in out.columns:
...
def write_group(group_name, latest_name, history_name):
    ...
    # Monthly history for state-transition diagnostics; full long-run
    # backtest is handled separately by backtest_sector_research.py.
    actual_month_ends = (
    ...
    hist_dates = actual_month_ends[-72:]
    hist = []
    for d in hist_dates:
        ...
        if len(part):
            hist.append(part)
    if hist:
        pd.concat(hist, ignore_index=True).to_csv(OUT / history_name, index=False)
    else:
        pd.DataFrame().to_csv(OUT / history_name, index=False)

    print(f"{group_name}: latest={len(latest)} rows; history months={len(hist_dates)}")


def main():
    write_group(
        "sector",
        "sector_research_features_latest.csv",
        "sector_research_features_history.csv",
    )
    write_group(
        "global_sector",
        "global_sector_research_features_latest.csv",
        "global_sector_research_features_history.csv",
    )
